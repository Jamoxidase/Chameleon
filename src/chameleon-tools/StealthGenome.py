...
class StealthGenome:
    ...
    def _fastaGenome(self,fasta: str):
        ...
        for frag,(header,seq) in enumerate(reader.readFasta()):
            genome = Seq(seq)
            orfs = ORFfinder(seq,longestGene=True,minGene=200).geneCandidates #array of estimated genes
    ...
    def __init__(self, genome_infile) -> None:
        ...
        if genome_infile != None:
            ...
            if genome_infile.endswith(".fasta"):
                self.genome_sequence,self.cds_sequence =  self._fastaGenome(genome_infile)
...
class PlasmidParser:
    ...
    def _parsePlasmid(self, gbfile: str) -> tuple[list[SimpleLocation],list[SimpleLocation],Seq]:
